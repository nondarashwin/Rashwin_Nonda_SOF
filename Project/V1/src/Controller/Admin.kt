...
class Admin {
    ...
    fun billing() {
        ...
        for (i in 0 until n) {
            ...
            if (product != null) {
                ...
                val stock = searchStock(product.productId, storeId)
                if (stock?.stock!! > product.productQuantity && product.productQuantity>0) {
                    products.add(product)
                    val a = stock.stock.minus(product.productQuantity)
                    stock.setterStock(a)
    ...
    private fun displayBill(id: Int, mailId:String) {
        ...
        var message="""Dear Customer,<br>this is your bill for today's shopping<br>
            <h5>Bill ID=$id</h5>
            <table><tr><th>Product Name</th><th>Quantity</th><th>Unit Cost</th><th>totals</th></tr>
                """
        for (i in bills) {
            ...
            if (i.billId == id) {
                ...
                for (j in i.productIds) {
                    val p=searchProduct(j.productId)
                    if (p != null) {
                        println("${p.productName} ${j.productQuantity} ${j.productCost} ${j.productQuantity * j.productCost}")
                        val temp=message
                        message=temp.plus("<tr><td>${p.productName}</td><th>${j.productQuantity}</th><th>${j.productCost}</th><th>${j.productQuantity * j.productCost}</th></tr>")
                    }
    ...
    private fun searchProduct(id: Int): Product? {
        ...
        for (i in product) {
            if (i.productId == id && i.productContniuity) {
                return i
            }
        }
        return null
    ...
    fun topPerformingStore() {
        ...
        if (s != null) {
            println("Store name=" + s.storeName)
            println("Store Address=" + s.storeAddress)
            ...
            if (i.stock <= 10) {
                ...
                if (product != null) {
                    if (store != null) {
                        ...
                        val temp = message.plus("Store Name=${store.storeName} Store Address=${store.storeAddress} Product Name=${product.productName} stock Remaining=${i.stock}<br> ")
                        message = temp
                    }
                }
            }
