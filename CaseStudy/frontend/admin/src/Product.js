...
class Product extends Component {
    ...
    componentDidMount() {
        let st1 = this.state
        st1.loading=true
       this.setState(st1,()=>{ axios.get("http://localhost:9090/api/products").then(res => {
                ...
                return this.state.Products.map((product) => {
                    ...
                    return (
                        ...
                        <div className="col-3" style={myStyle} key={product.id}>
                            <div className="card"><h5 className="card-title">{product.name}</h5><img
    ...
    render() {
        return this.state.loading ? <div style={{align:"center",margin: 0,position: "absolute",top:"50%",left: "50%"}}><Loading/></div> : this.output()
    }
}
