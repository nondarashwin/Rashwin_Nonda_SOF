...
public class StockController {
    ...
    private final String topic ="mails";
    //private String topicLogger="trackers";
    @Autowired
    ...
    public void sendMailToStore(StorePassword storePassword){
        ...
        if (!storePassword.getMailId().matches(emailPattern)) {
        ...
        kafkaTemplate.send(topic,storePassword.getMailId()+","+ "Welcome To ZippQ"+","+ message);
        //sendMail.sendMail(storePassword.getMailId(),"Welcome To Zippq",message);
    }
    public void sendMailToCustomer(Bill bill) {
        ...
        kafkaTemplate.send(topic,bill.getEmailId()+","+ "Bill"+","+ message+"<tr><th>  </th>  <th>  </th><th>  </th><th>Amount=" + bill.getTotalAmount() + "</th></tr></table><br>yours sincerely<br>Your Favourite Store ");
    ...
    public ResponseEntity<StorePassword> resetPassword(@PathVariable int id){
        StorePassword storePassword=storePasswordRepository.findById(id).orElseThrow(()-> new StocksException("id not found"));
        ...
        headers.add("Response-from", "SetPassword");
...
public List sales(){
    ...
    public ResponseEntity<StorePassword> getStore(@PathVariable int id){
        StorePassword storePassword1=storePasswordRepository.findById(id).orElseThrow(()->new StocksException("User Id not found"));
        HttpHeaders headers = new HttpHeaders();
        headers.add("Response-from", "ProductValue");
